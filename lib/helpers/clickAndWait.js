 ...
 * @param { string } elementSelector - CSS Selector to click on.
 ...
 */
async function clickAndWait(elementSelector, { timeout = 1000, selector = '#noExist', customFunction = async (timeout) => await page.waitForTimeout(timeout) } ={}) {
    try {
        await page.click(elementSelector);
    } catch(e) {
        Logger.warn('clickAndWait', { elementSelector }, 'Using the click method of Puppeteer failed with the following message: ' + e.message);
        Logger.info('clickAndWait', { }, 'Making click using the click method on element.');
        const browserButton = document.querySelector(elementSelector);
        if(browserButton) {
            browserButton.click();
        } else {
            Logger.warn('clickAndWait', { elementSelector }, 'Using the Browser click method failed because the selector do not match any element');
        }
    } finally {
        await Promise.any([
