/*
 Copyright 2021 Plato Solutions, Inc.

 Licensed under the Apache License, Version 2.0 (the "License");
 you may not use this file except in compliance with the License.
 You may obtain a copy of the License at

      https://www.apache.org/licenses/LICENSE-2.0

 Unless required by applicable law or agreed to in writing, software
 distributed under the License is distributed on an "AS IS" BASIS,
 WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 See the License for the specific language governing permissions and
 limitations under the License.
 */

import Collector from '../collector.js';
import Collection from '../collection/collection.js';
import CollectionOptionProcessor from "../collection/processors/option.js";
import Query from "../query/query.js";
import SelectorInterpreter from '../query/interpreter/selector.js';
import Option from '../option/option.js';

/**
 * Creates a Collector instance that returns a list of combinations
 * generated by Options intances.
 */
class OptionCollectorFactory {

    /**
     * @param { object } options - A series of options.
     * @returns { Collector } A Collector instance.
     */
    constructor(options) {
        this.options = options;
        return this.#createCollector();
    }

    /**
     * Returns a Collector instance.
     * @returns { Collector } A Collector instance.
     */
    #createCollector() {
        
        const options = OptionCollectorFactory.#normalizeOptions(this.options);

        const optionCollector = new Collector(
            new Collection(
                options
            ).postProcessor(CollectionOptionProcessor)
        );

        OptionCollectorFactory.#addIterationQueries(optionCollector, options);

        return optionCollector;
        
    }

    /**
     * Execute normalization actions for each of the options.
     * For example, adding default values in queries.
     * @param { object } options - Set of options.
     * @returns { object } The normalized options.
     */
    static #normalizeOptions(options) {
        let result = {};

        for(let option in options) {
            
            let query = options[option];
            
            if(query instanceof Option) {
                Object.assign(result, { [option]: async function wrapper() { return query } });
            }else if(typeof query === 'string') {
                Object.assign(result, { [option]: new SelectorInterpreter(query).default(null) });
            } else if(query instanceof Query) {
                Object.assign(result, { [option]: query.default(null) });
            } else {
                Object.assign(result, { [option]: query });
            }

        }

        return result;
        
    }

    /**
     * Add queries to the Collector instance to extract
     * each of the options in the Collector run.
     * @param { Collector } collector - Collector instance.
     * @param { object } options - Set of options.
     */
    static #addIterationQueries(collector, options) {

        Object.assign(collector.iterationQuery, {
            meta: async function getElementMetaData() {
                return this.getElement();
            }
        });

        for(const identifier in options) {
            const query = { [identifier]: async function getOptionValue() {
                    const context = this.getElement();

                    return context.map(option => {
                        return option?.[identifier]?.["tagName"] === 'INPUT'?option?.[identifier]?.["checked"]:option?.[identifier]?.["innerText"]
                    }).filter(el => el)[0];
                }
            };

            Object.assign(collector.iterationQuery, query);
        }

    }
}

export default OptionCollectorFactory;