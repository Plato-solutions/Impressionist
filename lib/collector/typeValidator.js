...
class TypeValidator {
     ...
     * ```
     */
    static check(value, type = 'string') {
        
        if(typeof type === 'string') {
            if(typeof value !== type) {
                throw new Error(`The value ${value} is not a type ${type}.`);
            }
        } else {
            if(!(value instanceof type)) {
                throw new Error(`${value.constructor.name} is not an instance of ${type.name}.`)
            }
        }
     ...
     * //TODO: Examples.
     */
    static deepCheck(value, type) {
        if(Array.isArray(value)) {
            for(let item of value) {
                TypeValidator.check(item, type);
            }
        } else if(typeof value === 'object' && value.constructor === Object) {
            for(let item in value) {
                TypeValidator.check(value[item], type);
            }
        }else {
            TypeValidator.check(value, type);
        }
    }
