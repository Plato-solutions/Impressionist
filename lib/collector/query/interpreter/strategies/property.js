 ...
 */
 class InterpreterPropertyStrategy {
    ...
    static interpret(selector) {
        ...
        return (normalizedElementSelector.startsWith('::element') ? new Pre((element) => [element]) : new Css(normalizedElementSelector).setGetElement(elementSelector.startsWith('document')?new Context():undefined)).property(property)[selector.endsWith('*')?'all':'single']();
    }
