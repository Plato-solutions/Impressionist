 ...
 * 
 * @summary Keep a record of Implementation subclasses.
 */
class SelectorDirectory {

    /**
     * An object that stores the constructors of each subclass of Implementation.
     ...
     * 
     * @param { Object } implementation - Constructor class such as Css, Property, Single, etc.
     * 
    ...
    static register(implementation) {
        ...
        const classAlias = implementation.name.toLowerCase();

        if(!SelectorDirectory.#registry[classAlias]) {
            SelectorDirectory.#registry[classAlias] = function(...args) {
                return new implementation(...args); // Avoid the use of new keyword in page.evaluate
            }
        }
