...
class CollectionOptionProcessor {
     ...
     * @param { object } elementCollection - A serie of elements or Options instances.
     * @returns { Promise<Generator> } An object that represents a promise object
    ...
    static async call(elementCollection) {
        ...
        for(let identifier in elementCollection) {
            ...
            let query = elementCollection[identifier];

            if(!(query instanceof Option)) {
                query = new Option(identifier, elementCollection[identifier])
            }
     ...
     * @param { Array<Option> } options - A list of the Option instances.
     * @returns { Promise<Generator> } An object that represents a promise object
