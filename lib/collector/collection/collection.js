...
class Collection {
     ...
     * @param { string | function | Query } queries.* - QUery.
     */
    constructor(queries) {
    ...
    async call(context) {
        ...
        for(let query in this.queries) {
            try {
                ...
                Logger.error('Collection', `${query} - Execution of query failed: ${e.message}`);
            }
        }
    ...
    /**
     * Execute a query.
     * @param { Query | string | function } query - Query.
     * @param { Context } context - Object that represents the context or the element
     * that is being passed on nested queries or instances executions.
     * @returns { Promise<*> } An object that represents a promise for the result of the execution of a query.
     */
     ...
     * @returns { Promise<*> } An object that represents a promise
    ...
    async #executePostProcessors(collection) {
        ...
        try {
            ...
            Logger.error('Collection', 'Execution of PostProcessor failed with the following message: ' + e.message);
        }
